  ...
  std::unique_ptr<SessionHandler> SessionHandler::createSession( const UserCredentials & credentials )
  {
    ...
    try
    {
        ...
        )
      {
        ...
        if( credentials.roles[0] == "Job Seeker"        ) return std::make_unique<Domain::Session::JobSeekerSession>       ( credentials );
        if( credentials.roles[0] == "Recruiter"         ) return std::make_unique<Domain::Session::RecruiterSession>       ( credentials );
        if( credentials.roles[0] == "Career Specialist" ) return std::make_unique<Domain::Session::CareerSpecialistSession>( credentials );
        if( credentials.roles[0] == "IT"                ) return std::make_unique<Domain::Session::ITSession>              ( credentials );
